...
func (pmq *PaymentMethodQuery) QueryCustomer() *PaymentCustomerQuery {
	query := (&PaymentCustomerClient{config: pmq.config}).Query()
...
func (pmq *PaymentMethodQuery) Clone() *PaymentMethodQuery {
	...
	return &PaymentMethodQuery{
		...
		withCustomer: pmq.withCustomer.Clone(),
...
func (pmq *PaymentMethodQuery) WithCustomer(opts ...func(*PaymentCustomerQuery)) *PaymentMethodQuery {
	query := (&PaymentCustomerClient{config: pmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pmq.withCustomer = query
	return pmq
}
...
func (pmq *PaymentMethodQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentMethod, error) {
	var (
		...
		_spec       = pmq.querySpec()
		loadedTypes = [1]bool{
			pmq.withCustomer != nil,
		}
	)
	if pmq.withCustomer != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pmq.withCustomer; query != nil {
		if err := pmq.loadCustomer(ctx, query, nodes, nil,
...
func (pmq *PaymentMethodQuery) loadCustomer(ctx context.Context, query *PaymentCustomerQuery, nodes []*PaymentMethod, init func(*PaymentMethod), assign func(*PaymentMethod, *PaymentCustomer)) error {
