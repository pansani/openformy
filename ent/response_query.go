...
func (rq *ResponseQuery) QueryAnswers() *AnswerQuery {
...
func (rq *ResponseQuery) Clone() *ResponseQuery {
	...
	return &ResponseQuery{
		...
		withAnswers: rq.withAnswers.Clone(),
...
func (rq *ResponseQuery) WithAnswers(opts ...func(*AnswerQuery)) *ResponseQuery {
	query := (&AnswerClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withAnswers = query
	return rq
}
...
//		GroupBy(response.FieldSubmittedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Response.Query().
//		Select(response.FieldSubmittedAt).
//		Scan(ctx, &v)
...
func (rq *ResponseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Response, error) {
	...
	if rq.withForm != nil || rq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withAnswers; query != nil {
		if err := rq.loadAnswers(ctx, query, nodes,
			func(n *Response) { n.Edges.Answers = []*Answer{} },
			func(n *Response, e *Answer) { n.Edges.Answers = append(n.Edges.Answers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResponseQuery) loadForm(ctx context.Context, query *FormQuery, nodes []*Response, init func(*Response), assign func(*Response, *Form)) error {
	...
	for i := range nodes {
		if nodes[i].form_responses == nil {
			continue
		}
		fk := *nodes[i].form_responses
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "form_responses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ResponseQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Response, init func(*Response), assign func(*Response, *User)) error {
	...
	nodeids := make(map[int][]*Response)
	for i := range nodes {
		if nodes[i].user_responses == nil {
			continue
		}
		fk := *nodes[i].user_responses
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_responses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ResponseQuery) loadAnswers(ctx context.Context, query *AnswerQuery, nodes []*Response, init func(*Response), assign func(*Response, *Answer)) error {
	...
	query.Where(predicate.Answer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(response.AnswersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.response_answers
		if fk == nil {
			return fmt.Errorf(`foreign-key "response_answers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "response_answers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
