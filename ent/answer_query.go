...
func (aq *AnswerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Answer, error) {
	...
	if aq.withResponse != nil || aq.withQuestion != nil {
		withFKs = true
	}
	if withFKs {
...
func (aq *AnswerQuery) loadResponse(ctx context.Context, query *ResponseQuery, nodes []*Answer, init func(*Answer), assign func(*Answer, *Response)) error {
	...
	for i := range nodes {
		if nodes[i].response_answers == nil {
			continue
		}
		fk := *nodes[i].response_answers
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "response_answers" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
